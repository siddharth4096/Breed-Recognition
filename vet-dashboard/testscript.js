...
if (logoutBtn) {
  logoutBtn.addEventListener("click", (e) => {
    ...
    if (confirmLogout) {
        sessionStorage.removeItem("isVetLoggedIn"); // clear flag
      // Redirect to vet login page
      window.location.href = "../vet-login-page.html";
    }
  });
}
...
function initializeChatbot() {
    ...
    const responses = {
        'fmd': 'Foot and Mouth Disease (FMD) symptoms include fever, blisters on tongue and hooves, excessive drooling, and lameness. Immediate isolation and veterinary consultation required.',
        'vaccination': 'Standard vaccination schedule: FMD vaccine every 6 months, Anthrax annually, Brucellosis as recommended. Always maintain cold chain.',
        'respiratory': 'Respiratory infections in cattle often present with coughing, nasal discharge, difficulty breathing. Check for pneumonia, administer appropriate antibiotics.',
        'skin': 'Common skin conditions include ringworm, mange, and bacterial infections. Proper hygiene and targeted treatment based on diagnosis.',
        'emergency': 'For emergencies: Assess vital signs, stabilize the animal, contact emergency services, document symptoms and treatment administered.',
        'default': 'I can help with veterinary questions about diseases, symptoms, treatments, vaccinations, and emergency procedures. Try asking about FMD, vaccination schedules, or respiratory infections.'
    };
    ...
    function sendMessage(input, messagesContainer, isMainChat = false) {
    ...
    function generateResponse(message) {
        ...
        if (lowerMessage.includes('fmd') || lowerMessage.includes('foot and mouth')) {
            return responses.fmd;
        } else if (lowerMessage.includes('vaccination') || lowerMessage.includes('vaccine')) {
            return responses.vaccination;
        } else if (lowerMessage.includes('respiratory') || lowerMessage.includes('cough') || lowerMessage.includes('breathing')) {
            return responses.respiratory;
        } else if (lowerMessage.includes('skin') || lowerMessage.includes('rash') || lowerMessage.includes('infection')) {
            return responses.skin;
    ...
    if (sendChatbot) {
        sendChatbot.addEventListener('click', () => {
            sendMessage(chatbotInput, chatbotMessages);
        });
    }
    ...
    // Event listeners for AI Insights chat
    if (sendChat) {
        sendChat.addEventListener('click', () => {
            sendMessage(chatInput, chatMessages, true);
        });
    }
...
window.addEventListener('error', function(e) {
    console.error('Vet Portal Error:', e.error);
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('/Breed-Recognition/sw.js',  { scope: "/Breed-Recognition/" })
            .then((registration) => {
